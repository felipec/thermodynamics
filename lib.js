...
function brent(f, a, b, macheps, t, maxiter) {
  ...
  while (Math.abs(m)>tol && fb!=0){
    ...
    fb=f(b);
    if (!validNumber(fb)){
      throw `Brent's method f(t) is NAN for t = ${b}`;
    }
    if (Math.abs(fb) < macheps){
      return b;
    }
    if (fb*fc>0){
    ...
    iter+=1;
    if (!validNumber(a)){
      throw "Brent's method a is NAN";}
    if (!validNumber(b)){
      throw "Brent's method b is NAN";}
    if (!validNumber(c)){
      throw "Brent's method c is NAN";}
    if (iter>maxiter){
      throw `Brent's method reached maximum number of steps of ${maxiter} `;}
    if (Math.abs(fb)< 2*macheps*Math.abs(b)){
      return b;
    }
  }
  return b;
